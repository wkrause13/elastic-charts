...
docker run --rm --init --name e2e-playwright-tests -e PORT=${PORT} -e ENV_URL=${ENV_URL} -e PLAYWRIGHT_HTML_REPORT=${PLAYWRIGHT_HTML_REPORT} -w /app/e2e -v $(pwd)/../:/app/ ${pw_image} yarn test:playwright "$@"
